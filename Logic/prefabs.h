...
public:
    ...
    inline void createMaterials() {
        ...
        boxMaterial         = MaterialManager::CreateMaterial(Material(SHADER_voxelShader, TEX_box, 0, 0xff));
        ballMaterial        = MaterialManager::CreateMaterial(Material(SHADER_voxelShader, TEX_ball, 0, 0xff));
    ...
    inline Object createPlayer(bool isClient) {
        ...
        player.transform.position = Vector3(0, 3, 20);

        player.AddComponent<RenderView>().mesh_index = 4;
        player.GetComponent<RenderView>().layout = 1;
        ...
        player.AddComponent<CharacterController>().mask = 1;
    ...
    Object createBox() {
		...
		box.AddComponent<Voxels>().destructible = true;
    ...
    Object createBall() {
		...
		ball.GetComponent<RenderView>().materals[0] = ballMaterial;
		ball.AddComponent<NetworkIdentity>();
		Voxels& voxels = ball.AddComponent<Voxels>();
        voxels.destructible = true;
        voxels.blowable = true;
        voxels.minP = -0.5f;
        voxels.maxP = 0.5f;
        memset(voxels.data, 0, sizeof(voxels.data));
        float center = (8 - 1) * 0.5f;
        float radius = 222.0f;
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                for (int z = 0; z < 8; z++) {

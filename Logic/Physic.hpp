...
inline void PhysicSystem::solve(SphereCollider& s1, SphereCollider& s2) {
    Vector3 posA = getCenterMass(s1);
    Vector3 posB = getCenterMass(s2);
...
inline void PhysicSystem::solve(SphereCollider& sphere, CubeCollider& cube) {
    Vector3 sphereCenter = getCenterMass(sphere);
...
inline void PhysicSystem::solve(SphereCollider& sphere, CapsuleCollider& capsule) {
    ...
    cp.normal = normal;
    cp.penetration = penetration;
    cp.point = (sphereCenter - normal * r1 + p1 + normal * r2) * 0.5f;
...
inline void PhysicSystem::solve(Collider* a, Collider* b)
{
    ...
    if (!init) {
        ...
        dispatch[(int)ColliderType::Sphere][(int)ColliderType::CharacterController] =  &PhysicSystem::solveAdapter<SphereCollider, CapsuleCollider>;
        dispatch[(int)ColliderType::Terrain][(int)ColliderType::CharacterController] =  &PhysicSystem::solveAdapter<TerrainCollider, CapsuleCollider>;
        dispatch[(int)ColliderType::Cube][(int)ColliderType::CharacterController] =  &PhysicSystem::solveAdapter<CubeCollider, CapsuleCollider>;
        dispatch[(int)ColliderType::Capsule][(int)ColliderType::CharacterController] =  &PhysicSystem::solveAdapter<CapsuleCollider, CapsuleCollider>;
        dispatch[(int)ColliderType::CharacterController][(int)ColliderType::CharacterController] =  &PhysicSystem::solveAdapter<CapsuleCollider, CapsuleCollider>;
        init = true;
    }
...
inline void PhysicSystem::simulatePhysicStep() {
	...
	for (int i = 0; i < rigidbodies.size(); i++)
    {
        ...
        CharacterController* cc = rb.object.HasComponent<CharacterController>() ? &rb.object.GetComponent<CharacterController>() : nullptr;
        if (cc) cc->grounded = false;
    ...
    for (int i = 0; i < aabb.size(); ++i) {
        ...
        for (int j = i + 1; j < aabb.size(); ++j) {
            ...
            Collider* A = &physicView[objA.index];
            Collider* B = &physicView[objB.index];
            bool dynamicA = A->object.HasComponent<Rigidbody>() && !A->object.GetComponent<Rigidbody>().isKinematic;
            bool dynamicB = B->object.HasComponent<Rigidbody>() && !B->object.GetComponent<Rigidbody>().isKinematic;
    ...
    for (int iter = 0; iter < iterations; iter++) {
        for (Contact& c : contacts) {
            ...
            Vector3 Apos = getCenterMass(*c.a);
            Vector3 Bpos = getCenterMass(*c.b);
            for(int i = 0; i < c.count; i++) {
                float penetration = c.points[i].penetration; 
                if (!std::isfinite(penetration) || penetration <= 0.0f) continue;
                ...
                CharacterController* ccA = c.a->object.HasComponent<CharacterController>() ? &c.a->object.GetComponent<CharacterController>() : nullptr;
                CharacterController* ccB = c.b->object.HasComponent<CharacterController>() ? &c.b->object.GetComponent<CharacterController>() : nullptr;
                if (ccA) {
                    Vector3 n = normal.normalized();
                    float up = Vector3::Dot(n, Vector3(0,1,0));
                    float limit = cos(ccA->slopeLimit * 3.1415926f / 180.0f);
                    if (up >= limit) {
                        ccA->grounded = true;
                        ccA->groundNormal = n;
                    }
                ...
                if (ccB) {
                    Vector3 n = (-normal).normalized();
                    float up = Vector3::Dot(n, Vector3(0,1,0));
                    float limit = cos(ccB->slopeLimit * 3.1415926f / 180.0f);
                    if (up >= limit) {
                        ccB->grounded = true;
                        ccB->groundNormal = n;
                    }
                ...
                if (velAlongNormal < 0.0f) {
                    Vector3 impulse = normal * (velAlongNormal / invMassSum);
                    if(A) A->velocity -= impulse * invMassA;
                    if(B) B->velocity += impulse * invMassA;
                }
                ...
                if(A) rv += A->velocity;
                if(B) rv -= B->velocity;
                Vector3 tangentVel = Vector3::ProjectOnPlane(rv, normal);
                float tangentSpeed = tangentVel.length();
                if (tangentSpeed > 0.0001f) {
                    Vector3 tangentDir = tangentVel / tangentSpeed;
                    float friction = 10.0f;
                    float maxStop = friction * dt;
                    float reduce = std::min(tangentSpeed, maxStop);
                    Vector3 frictionImpulse = tangentDir * (reduce / invMassSum);
                    if(A) A->velocity -= frictionImpulse * invMassA;
                    if(B) B->velocity += frictionImpulse * invMassA;
                }
                ...
                // Vector3 rv(0,0,0);
                // if(A) rv += A->velocity + Vector3::Cross(A->angularVelocity, ra);
                // if(B) rv -= B->velocity + Vector3::Cross(B->angularVelocity, rb);
                // float velAlongNormal = Vector3::Dot(rv, normal);
                // if(velAlongNormal < -0.0f) {
    ...
    for(int i =0; i < rigidbodies.size(); i++) {
        Rigidbody& rb = rigidbodies[i];
        if (rb.isKinematic) continue;
        rb.object.transform.position = rb.prevPosition + rb.velocity * dt;
    }
}
